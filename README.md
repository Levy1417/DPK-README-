...
・DynamicActionで値を制御する場合に備えて、json_objプロパティとsilent_update_json_widget関数を外部からアクセスできるように変更
　このノードで動的生成されたウィジェットの値を変更する場合は、json_objの値も同時に更新したあと、silent_update_...関数でjsonウィジェットの文字列に反映するようにしてください。
