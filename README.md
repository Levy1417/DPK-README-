...
あるいは、JavascriptもPythonもGithubもよく分からない状態から勢いで作り、よく分からないまま公開してしまった特級呪物の所蔵庫ともいいます。<br>
SteamからEarly Accessの謎ゲーをダウンロードする気分で試してやってください。<br>
なお、力尽きたのでメンテナンスはあまりできません。<br>
<br>
<h2>カスタムノードの特徴</h2>
...
- 実行時処理の場合、ノードの処理順が計算される前に状態を切り替えるように設計することで、他の同様のノードで見られるような状態指定の「すり抜け」を防止します。<br>
...
・ 最初の画像で型名の後に示されている「v1,v2,v3...」のような文字列は、statementsに入力するコードで中身を参照する時の変数名となります。<br>
...
　このutilsはmodules/anyeval_utils.pyで定義したモジュールを参照するための変数であり、何度も現れる処理をanyeval_utils.pyに定義しておくことにより、<br>
  毎度コードの一部をどこかからコピペしてきて問題が起こるたびに関連する入力をすべて修正して回るという地獄を見なくて済みます。<br>
<br>
...
★ 入力について<br>
  ...
  　AnyEvalの入力側のスロットと変数の対応関係に基づいて、このノードのoutput typeと出力スロットを自動で設定します。<br>
  ★ output typeウィジェットについて<br>
...
・ComfyNodeオブジェクトからは、inputsプロパティで各入力スロット、outputsで各出力スロット、widgetsで各ウィジェットの配列を取得することができます。<br>
 詳細については、色々なカスタムノードのソースコード（.jsファイル）を参照してみてください。<br>
 このカスタムノードのソースコードを読んでも構いませんが、仕様も言語もコード管理も分からないまま格闘した結果地獄のコードになっていますので、その点だけ了承お願いします。<br>
